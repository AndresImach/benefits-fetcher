...
const mongo_collection = 'BBVA_GO'
...
async function run() {
  ...
  try {
    ...
    while (continueFetching) {
      await makeRequestWithBackoff(`https://go.bbva.com.ar/fgo/API/v1/communications?pager=${pager}&destacado=${destacado}`)
        .then((res) => {
          console.log(`pager ${pager}, destacado ${destacado}`)
          ...
          if (resData.data.length < 1) {
            if (destacado === 's') {
              destacado = 'n'
              pager = 0
              console.log("changing to destacado = n")
              return
            } else {
        ...
        .catch(error => {
          console.log(`Error fetching data for pager=${pager}&destacado=${destacado}: ${error.message}`);
        })
    }
    ...
    for (let id of benefitsIds) {
      await makeRequestWithBackoff(`https://go.bbva.com.ar/fgo/API/v1/communication/${id}`)
        .then((res) => {
          console.log(`Getting benefit ${id}`)
          let data = res;
          okArray.push(data.data)
        })
        .catch(error => {
